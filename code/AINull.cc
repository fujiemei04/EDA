    ...
    if (u.damage == 0 and virus_free) {
      ...
      while (not q.empty() and not found) {
          ...
          for (int d = 0;d < 4;++d) {
            Pos p2 = p1+Dir(d);
            Cell c = cell(p2);
            if (pos_ok(p2) and not c.mask and c.virus==0 and dist[p2.i][p2.j] == -1 and c.type != WALL) {
      ...
      if (found) {
        ...
        Pos p4 = resultat[1];
        if ((p4.i - p3.i) == 0) {
        if ((p4.j - p3.j) == -1) move(id,BOTTOM);
          else move(id,TOP);
        }
        else if ((p4.j - p3.j) == 0) {
          if ((p4.i - p3.i) == -1) move(id,LEFT);
          else move(id,RIGHT);
        }
      }
      ...
      if (round() >= 5) {
        ...
        Pos p1;
        if (jugador_enemic(res[0],id,p1)) {
          if ((p1.i - res[0].i) == 0) {
            if ((p1.j - res[0].j) == -1) move(id,BOTTOM);
            else move(id,TOP);
          }
          else if ((p1.j - res[0].j) == 0) {
            if ((p1.i - res[0].i) == -1) move(id,LEFT);
            else move(id,RIGHT);
          }
        }
        else if (found) {
